<template>
  <div class="v-table-dynamic" ref="tableWrapper">
    <div
...
<script>
import {is2DMatrix, unemptyArray} from '../utils/array.js'
...
export default {
    ...
    columnWidth() {
      if (this.params && unemptyArray(this.params.columnWidth)) {
        ...
        if (percentageList.length > 0) {
          let total = percentageList.reduce((t, p) => {
            return parseFloat(t) + parseFloat(p)
          })
          if (total > 100) {
      ...
      if (this.params && unemptyArray(this.params.filter)) {
        ...
        this.params.filter.forEach(f => {
          if (f && typeof f.column === 'number' && f.column >= 0 && typeof f.method === 'function' && unemptyArray(f.content)) {
            if (f.content.every(c => {
              return (c && typeof c.text === 'string' && typeof c.value !== 'undefined')
            })) {
  ...
  methods: {
    ...
    updateColumnWidth() {
      ...
      if (this.sharedWidth && keys.every(k => {
        return this.columnWidth[k] && this.columnWidth[k].value && this.columnWidth[k].type === 'absolute'
      })) {
        ...
        keys.forEach(k => {
          tableContentWidth += parseFloat(this.columnWidth[k].value)
        })
